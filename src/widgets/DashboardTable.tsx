...
export function DashboardTable({ coins, query, sortKey, sortDir, onSort }: Props) {
	const q = query.trim().toLowerCase()
	const visible = coins.filter((c) => c.name.toLowerCase().includes(q) || c.symbol.toLowerCase().includes(q))
			...
			<TBody>
				{visible.map((c) => (
					<tr key={c.id}>
						...
						<Td className={c.price_change_percentage_24h >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
							{c.price_change_percentage_24h.toFixed(2)}%
						</Td>
						<Td>
							<div className="h-10 w-32">
								<LineChart data={(c.sparkline_in_7d?.price || []).map((y, i) => ({ x: i, y }))} />
							</div>
						</Td>
					</tr>
				))}
